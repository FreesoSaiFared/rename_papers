...
import shutil  # For checking if executables exist
import os      # For environment variables and file paths

# =====  CONSTANTS

# Name of the PDF-to-text executable
PDF_TO_TEXT = "pdftotext.exe"  # Adjust for Windows executable
...
def get_fragments(infile):
 ...
 if shutil.which(PDF_TO_TEXT):
         ...
         logging.warning("pdftotext not found. Attempting installation...")
 
         # Try installing with winget
         if shutil.which("winget"):
             logging.info("Trying winget...")
             try:
                 run(["winget", "install", "poppler"], check=True)
             except CalledProcessError as e:
                 logging.warning("winget installation failed:", e)
 
         # If winget fails or is not available, try choco
         if not shutil.which(PDF_TO_TEXT) and shutil.which("choco"):
             logging.info("Trying choco...")
             try:
                 run(["choco", "install", "poppler"], check=True)
             except CalledProcessError as e:
                 logging.warning("choco installation failed:", e)
 
         # If both winget and choco fail or are not available, inform the user
         if not shutil.which(PDF_TO_TEXT):
             logging.error("Could not install pdftotext. Please install it manually.")
             return ["--error, pdftotext not installed--"]
 
         # Retry conversion after potential installation
         return get_fragments(infile)  # Recursive call to retry
